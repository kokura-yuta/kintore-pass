...
export default function FoodScreen() {
  ...
  async function saveEntry() {
    ...
    try {
      ...
      if (isApiBypassEnabled) {
        savedRecord = { id: editingId ?? `food-${Date.now()}-${Math.random().toString(36).slice(2)}`, ...input };
        setLocalEntries((current) => editingId ? current.map((item) => item.id === editingId ? savedRecord : item) : [...current, savedRecord]);
      } else {
        const token = await getTokenRef.current();
        if (!token) throw new Error('ログイン状態を確認できませんでした。');
        savedRecord = editingId ? await updateFoodRecord(token, editingId, input) : await createFoodRecord(token, input);
      }
    ...
    } finally { savingLock.current = false; setIsSaving(false); }
  }
  ...
  async function removeEntry() {
    ...
    try {
      if (isApiBypassEnabled) setLocalEntries((current) => current.filter((entry) => entry.id !== recordId));
        ...
        const token = await getTokenRef.current();
        if (!token) throw new Error('ログイン状態を確認できませんでした。');
        await deleteFoodRecord(token, recordId);
      }
      setEntries((current) => current.filter((entry) => entry.id !== recordId));
      ...
      setDeletingId(null);
    } finally { deletingLock.current = false; setIsDeleting(false); }
  }
...
const styles = StyleSheet.create({
  ...
  input: { minHeight: 50, paddingHorizontal: 14, borderWidth: 1, borderColor: '#294653', borderRadius: 13, color: '#F4F6F3', backgroundColor: '#071018', fontSize: 14 }, numberRow: { flexDirection: 'row', gap: 10 }, numberField: { flex: 1 }, inputWithUnit: { minHeight: 50, flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#294653', borderRadius: 13, backgroundColor: '#071018' }, numberInput: { flex: 1, minWidth: 0, paddingHorizontal: 13, color: '#F4F6F3', fontSize: 14 }, inputUnit: { paddingRight: 12, color: '#70838E', fontSize: 10, fontWeight: '700' },
