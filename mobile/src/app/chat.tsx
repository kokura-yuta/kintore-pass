        ...
        >
          ...
          <View style={styles.inputArea}>
            <View style={styles.inputWrap}>
              <TextInput accessibilityLabel="AIコーチへの質問" multiline onChangeText={(value) => { setInput(value); setError(''); setFailedMessage(''); }} placeholder="筋トレについて相談する" placeholderTextColor="#657681" style={[styles.input, isTooLong && styles.inputError]} textAlignVertical="top" value={input} />
              <Text style={[styles.characterCount, isTooLong && styles.countError]}>{input.length}/{MAX_MESSAGE_LENGTH}</Text>
            </View>
            <Pressable disabled={!canSend} onPress={() => void submitMessage(input, true)} style={[styles.sendButton, !canSend && styles.disabledSend]}>{isSending ? <ActivityIndicator color="#050A0F" size="small" /> : <Text style={styles.sendText}>↑</Text>}</Pressable>
          </View>
...
function StatusPanel({ label }: { label: string }) {
  return <View style={styles.statusPanel}><ActivityIndicator color="#00D4FF" size="small" /><Text style={styles.statusText}>{label}</Text></View>;
}
...
const markdownStyles = StyleSheet.create({
  ...
  heading1: { color: '#73E7FF', fontSize: 20, lineHeight: 27, fontWeight: '800', marginTop: 8, marginBottom: 10 }, heading2: { color: '#73E7FF', fontSize: 18, lineHeight: 25, fontWeight: '800', marginTop: 8, marginBottom: 8 }, heading3: { color: '#00D4FF', fontSize: 16, lineHeight: 23, fontWeight: '700', marginTop: 6, marginBottom: 7 }, heading4: { color: '#00D4FF', fontSize: 14, lineHeight: 21, fontWeight: '700' }, heading5: { color: '#00D4FF', fontSize: 13, lineHeight: 20, fontWeight: '700' }, heading6: { color: '#00D4FF', fontSize: 12, lineHeight: 19, fontWeight: '700' },
  strong: { color: '#73E7FF', fontWeight: '800' }, bullet_list: { marginTop: 2, marginBottom: 10 }, ordered_list: { marginTop: 2, marginBottom: 10 }, list_item: { marginBottom: 5, flexShrink: 1 }, bullet_list_icon: { color: '#00D4FF', marginRight: 8 }, bullet_list_content: { flex: 1 }, ordered_list_icon: { color: '#00D4FF', marginRight: 8 }, ordered_list_content: { flex: 1 },
  table: { width: '100%', maxWidth: '100%', marginVertical: 10, borderWidth: 1, borderColor: '#1E6076' }, thead: { backgroundColor: '#082733' }, th: { flex: 1, padding: 7, color: '#73E7FF', borderColor: '#1E6076' }, tr: { flexDirection: 'row', borderBottomWidth: 1, borderColor: '#19495A' }, td: { flex: 1, flexShrink: 1, padding: 7, color: '#E8EBE8', borderColor: '#19495A' },
  code_inline: { color: '#A8F2FF', backgroundColor: '#08131A', borderColor: '#29434E', borderWidth: 1, borderRadius: 4, paddingHorizontal: 5 }, fence: { maxWidth: '100%', color: '#E8EBE8', backgroundColor: '#061017', borderColor: '#233943', borderWidth: 1, borderRadius: 7, padding: 10, fontFamily: Platform.select({ ios: 'Menlo', android: 'monospace', default: 'monospace' }), fontSize: 11, lineHeight: 18 }, code_block: { maxWidth: '100%', color: '#E8EBE8', backgroundColor: '#061017', borderColor: '#233943', borderWidth: 1, borderRadius: 7, padding: 10, fontFamily: Platform.select({ ios: 'Menlo', android: 'monospace', default: 'monospace' }), fontSize: 11, lineHeight: 18 }, blockquote: { backgroundColor: '#0B161E', borderLeftColor: '#00D4FF', borderLeftWidth: 3, paddingHorizontal: 10, paddingVertical: 6 }, link: { color: '#A8F2FF', textDecorationLine: 'underline' },
});

const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#050A0F' }, safeArea: { flex: 1 }, header: { minHeight: 66, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, borderBottomWidth: 1, borderBottomColor: '#142833' }, menuButton: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center' }, menuIcon: { color: '#F4F6F3', fontSize: 20 }, headerCopy: { flex: 1, marginHorizontal: 7 }, eyebrow: { color: '#73E7FF', fontSize: 8, fontWeight: '700', letterSpacing: 1.2 }, title: { marginTop: 3, color: '#F4F6F3', fontSize: 16, fontWeight: '700' }, newButton: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: '#294653', borderRadius: 12 }, newButtonText: { color: '#73E7FF', fontSize: 20 },
  ...
  errorRow: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 14, paddingVertical: 7, backgroundColor: '#201516' }, errorText: { flex: 1, color: '#FF8D98', fontSize: 11, lineHeight: 16 }, retrySmall: { paddingHorizontal: 10, paddingVertical: 7, borderWidth: 1, borderColor: '#00D4FF', borderRadius: 8 }, retrySmallText: { color: '#73E7FF', fontSize: 10, fontWeight: '700' }, inputArea: { flexDirection: 'row', alignItems: 'flex-end', gap: 9, paddingHorizontal: 13, paddingTop: 10, paddingBottom: 8, borderTopWidth: 1, borderTopColor: '#142833', backgroundColor: '#081018' }, inputWrap: { flex: 1 }, input: { maxHeight: 130, minHeight: 48, paddingHorizontal: 14, paddingTop: 12, paddingBottom: 19, borderWidth: 1, borderColor: '#294653', borderRadius: 15, backgroundColor: '#050A0F', color: '#F4F6F3', fontSize: 13, lineHeight: 19 }, inputError: { borderColor: '#FF7676' }, characterCount: { position: 'absolute', right: 10, bottom: 5, color: '#657681', fontSize: 9 }, countError: { color: '#FF7676' }, sendButton: { width: 48, height: 48, alignItems: 'center', justifyContent: 'center', borderRadius: 15, backgroundColor: '#00D4FF' }, disabledSend: { opacity: 0.35 }, sendText: { color: '#050A0F', fontSize: 20, fontWeight: '700' }, lengthError: { paddingHorizontal: 14, paddingBottom: 7, color: '#FF7676', backgroundColor: '#081018', fontSize: 10 },
  modalBackdrop: { flex: 1, flexDirection: 'row', backgroundColor: 'rgba(0,0,0,0.68)' }, modalDismiss: { flex: 1 }, drawer: { width: '86%', maxWidth: 380, backgroundColor: '#081018' }, drawerHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: 18, borderBottomWidth: 1, borderBottomColor: '#142833' }, drawerTitle: { color: '#F4F6F3', fontSize: 20, fontWeight: '700' }, closeText: { color: '#8798A3', fontSize: 26 }, drawerNewButton: { margin: 14, padding: 13, borderWidth: 1, borderColor: '#00D4FF', borderRadius: 13 }, drawerNewText: { color: '#73E7FF', fontSize: 12, fontWeight: '700', textAlign: 'center' }, chatList: { flexGrow: 1, paddingHorizontal: 12, paddingBottom: 20 }, emptyHistory: { padding: 16, color: '#8798A3', fontSize: 11, lineHeight: 18, textAlign: 'center' }, chatRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 6, borderRadius: 12, backgroundColor: '#0C1820' }, activeChatRow: { backgroundColor: '#082733' }, chatSelect: { flex: 1, padding: 12 }, chatTitle: { color: '#E8EBE8', fontSize: 12, fontWeight: '600' }, chatMeta: { marginTop: 4, color: '#7D8E99', fontSize: 9 }, deleteButton: { minWidth: 52, minHeight: 46, alignItems: 'center', justifyContent: 'center', padding: 9 }, deleteText: { color: '#FF8D98', fontSize: 10, fontWeight: '600' }, drawerNote: { padding: 15, color: '#657681', fontSize: 9, textAlign: 'center' },
});
