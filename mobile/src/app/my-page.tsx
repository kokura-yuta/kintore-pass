        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.safeArea}>
          <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            <Pressable disabled={isSaving} onPress={saveProfile} style={[styles.saveButton, isSaving && styles.disabledButton]}>
              {isSaving ? <ActivityIndicator color="#0B0D0C" /> : <Text style={styles.saveText}>プロフィールを保存</Text>}
            </Pressable>
...
const styles = StyleSheet.create({
  ...
  title: { marginTop: 5, color: '#F4F6F3', fontSize: 29, fontWeight: '900' },
  ...
  goalValue: { marginTop: 6, color: '#B6F24B', fontSize: 14, fontWeight: '900' },
  outlineButton: { paddingHorizontal: 14, paddingVertical: 9, borderWidth: 1, borderColor: '#B6F24B', borderRadius: 11 },
  outlineButtonText: { color: '#B6F24B', fontSize: 11, fontWeight: '900' },
  measurementRow: { flexDirection: 'row', gap: 10, marginTop: 17 },
  singleField: { marginTop: 17 },
  optionTitle: { marginTop: 20, color: '#C9CECA', fontSize: 11, fontWeight: '800' },
  optional: { color: '#697169', fontSize: 9 },
  chipRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 9 },
  chip: { minWidth: 55, alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, borderWidth: 1, borderColor: '#343A35', borderRadius: 20, backgroundColor: '#0B0D0C' },
  selectedChip: { borderColor: '#B6F24B', backgroundColor: '#B6F24B' },
  chipText: { color: '#A5ADA7', fontSize: 11, fontWeight: '800' },
  selectedChipText: { color: '#0B0D0C' },
  success: { marginTop: 13, color: '#B6F24B', fontSize: 12, fontWeight: '800' },
  saveButton: { minHeight: 55, alignItems: 'center', justifyContent: 'center', marginTop: 14, borderRadius: 15, backgroundColor: '#B6F24B' },
  disabledButton: { opacity: 0.5 },
  saveText: { color: '#0B0D0C', fontSize: 15, fontWeight: '900' },
